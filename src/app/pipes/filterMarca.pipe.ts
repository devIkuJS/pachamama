...
export class FilterByMarcaPipe implements PipeTransform {
      ...
      if (isAnd) {
        return items.filter(item =>
            filterKeys.reduce((acc1, keyName) =>
                ...
                acc2 && new RegExp(item[keyName], 'gi').test(checkedItem.idmarca) || checkedItem.idmarca === "", true))
              , true)
              );
      } else {
        return items.filter(item => {
          return filterKeys.some((keyName) => {
            return checkedItems.some((checkedItem) => {
              return new RegExp(item[keyName], 'gi').test(checkedItem.idmarca) || checkedItem.idmarca === "";
            });
          });
        });
      }
